...
func TestBuiltinToolsSmoke(t *testing.T) {
	...
	call(t, EditFile(dir), `{"path":"notes.txt","old_string":"beta","new_string":"BETA"}`)
	...
	call(t, EditFile(dir), `{"path":"notes.txt","start_marker":"alpha","end_marker":"gamma","new_string":"only"}`)
	data, _ = os.ReadFile(filepath.Join(dir, "notes.txt"))
	...
	_, err := EditFile(dir).Handler(context.Background(), json.RawMessage(`{"path":"dup.txt","old_string":"x","new_string":"y"}`))
	if err == nil {
