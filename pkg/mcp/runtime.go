...
// ToolHit 是 MCP 工具检索的命中视图（字段与 mcp.ToolHit 平行定义：
// tools 包不 import pkg/mcp，保持 tools → 无任何能力源包的单向依赖）。
type ToolHit struct {
...
// 与 SkillRuntime 同模式：接口在消费侧定义，实现在装配层桥接。
type MCPProvider interface {
	...
	// MaterializedNames 返回本会话已注册的 MCP 工具名（状态栏 loaded
	// 区 tools: 集合的数据源）。
