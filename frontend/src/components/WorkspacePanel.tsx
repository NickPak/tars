    ...
    <aside className="workspace">
      <div className="workspace-section-header">
        ...
        <div className="workspace-section-actions">
          <button className="ws-icon-btn" title="历史项目">
            <History size={16} />
          </button>
          <button
          ...
          <button className="ws-icon-btn" title="折叠全部" onClick={collapseAll}>
          ...
          <button className="ws-icon-btn" title="新建项目">
            <Plus size={16} />
          </button>
          <button
            className="ws-icon-btn"
...
function FilesTab() {
  ...
  const refresh = useWorkspaceStore((s) => s.refresh);
  const collapseKey = useWorkspaceStore((s) => s.collapseKey);
  ...
  // 折叠全部
  useEffect(() => {
    if (collapseKey > 0) setExpanded(new Set());
  }, [collapseKey]);
  ...
  const handleOpenFile = (relPath: string) => {
    if (!activeId) return;
    void agentApi.openFile(activeId, relPath);
  };

  const handleRevealFile = (relPath: string) => {
    if (!activeId) return;
    void agentApi.revealFileInExplorer(activeId, relPath);
    setCtxMenu(null);
  };
