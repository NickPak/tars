...
// mcpRuntime 是 tools.MCPRuntime 的实现（装配层桥接）：闭包捕获会话级
// tools.Registry，把命中的 MCP 工具包装为 Definition 动态注册进本会话
// （plan 3.6 D1 动态注册决策）。与 skillRuntime 同模式：接口在 tools
// 包定义，实现在装配层。
type mcpRuntime struct {
	mgr     *mcp.Manager
	toolReg *tools.Registry // 会话级注册表（动态注册的归宿）
...
var _ mcp.MCPProvider = (*mcpRuntime)(nil)

// mcpConnectTimeout 是懒启动握手超时（npx 类启动器首次下载可能较慢，
// 但注册发生在轮内，必须有界）。
const mcpConnectTimeout = 60 * time.Second

// newMCPRuntime 创建会话级 MCP 通道；mgr 为 nil 时返回 nil（MCP 是可选能力，
// Env.MCP 保持 nil，discover_tools 只检索技能）。
func newMCPRuntime(mgr *mcp.Manager, toolReg *tools.Registry) *mcpRuntime {
	if mgr == nil {
		return nil
	}
	return &mcpRuntime{
...
func (r *mcpRuntime) Search(query string, limit int) ([]mcp.ToolHit, error) {
...
func (r *mcpRuntime) Materialize(hit mcp.ToolHit) error {
	r.mu.Lock()
	if r.materialized[hit.FullName] {
		r.mu.Unlock()
		return nil
	}
	r.mu.Unlock()
	...
	// 包装 Definition：handler 转发 MCP call（参数原文透传），
	// 文本结果提取回传；风险级别按服务器配置声明（审批门按声明拦截）。
	server, toolName := hit.Server, hit.Name
	def := &tools.Definition{
		Name:        hit.FullName,
		Description: mcpToolDescription(hit),
		Parameters:  hit.InputSchema,
		Risk:        r.serverRiskLevel(server),
		Handler: func(ctx context.Context, raw json.RawMessage) (string, error) {
			res, err := r.mgr.CallTool(ctx, server, toolName, raw)
			if err != nil {
				return "", err
			}
	...
	r.toolReg.Register(def)

	r.mu.Lock()
	r.materialized[hit.FullName] = true
	r.mu.Unlock()
	return nil
}

// MaterializedNames 返回本会话已注册的 MCP 工具名（排序稳定）。
func (r *mcpRuntime) MaterializedNames() []string {
	r.mu.Lock()
	out := make([]string, 0, len(r.materialized))
	for name := range r.materialized {
		out = append(out, name)
	}
	r.mu.Unlock()
	sort.Strings(out)
...
// serverRiskLevel 读取服务器配置的风险声明，映射到 tools.RiskLevel；
// 未配置/未知服务器回退 medium（失败安全默认值：宁可多审批一次）。
func (r *mcpRuntime) serverRiskLevel(server string) tools.RiskLevel {
