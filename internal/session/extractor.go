...
// 六条硬约束逐字写入，不允许实现者删减（02 篇 §6）。
// 注意：输出仍要求 JSON（解析需要），但**进上下文的形态是紧凑文本**
// （renderEntries）——存储格式与视图格式刻意解耦。
func buildExtractPrompt(req *ExtractRequest) string {
	...
	b.WriteString(`{"range":"...","goal":"...","userAsks":["..."],"actions":["..."],"result":"...",`)
	b.WriteString(`"facts":["..."],"pointer":"`)
	...
	b.WriteString(`2. "userAsks": quote the user's OWN WORDS verbatim — the essential sentence(s) of each `)
	b.WriteString("request in these turns. Do NOT paraphrase and do NOT translate. Everything else can be ")
	b.WriteString("rebuilt later by re-reading files, but the user's intent cannot; a paraphrase drifts ")
	b.WriteString("further with every later compression.\n")
	b.WriteString(`3. "facts": copy VERBATIM every value a later tool call must reproduce exactly — file `)
	b.WriteString("paths, URLs, UUIDs, hashes, IPs with ports, PR numbers, commit hashes. A single wrong ")
	b.WriteString("character breaks the call.\n")
	b.WriteString("4. Keep failed paths: what was tried, which approach failed and why the plan changed — ")
	b.WriteString("this prevents repeating mistakes.\n")
	b.WriteString(`5. Key-value style, no prose paragraphs; <= 200 tokens per entry excluding "userAsks" `)
	b.WriteString("(never trim a user request to fit the budget).\n")
	b.WriteString("6. Set \"pointer\" of every entry to exactly: ")
	b.WriteString(req.Pointer)
