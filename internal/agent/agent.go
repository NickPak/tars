...
// serve multiple conversations as long as messages/hooks are passed per-call.
type Agent struct {
...
func New(model model.ToolCallingChatModel, executor *tools.Manager, maxIterations int) *Agent {
	if maxIterations <= 0 {
		maxIterations = 1
	}
	return &Agent{
