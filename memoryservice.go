...
func (s *MemoryService) ListMemoryFacts() (*MemoryFactsView, error) {
	memMgr := boot.GetApp().GetMemoryMgr()
	...
	for _, pv := range boot.GetApp().ListProjects() {
...
func (s *MemoryService) ForgetMemoryFact(scope, projectID, subject string) error {
	memDir, err := boot.GetApp().GetMemoryDir(scope, projectID)
	if err != nil {
		return err
	}
...
func (s *MemoryService) AdoptMemoryCandidate(projectID, subject string) error {
	memDir, err := boot.GetApp().GetMemoryDir(string(memory.ScopeProject), projectID)
	if err != nil {
		return err
	}
...
func (s *MemoryService) ListMemoryAudit(scope, projectID string) (*MemoryAuditView, error) {
	root, err := boot.GetApp().GetMemoryDir(scope, projectID)
	if err != nil {
		return nil, err
	}
