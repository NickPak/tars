...
func (s *ModelService) GetModelInfo() (*ModelInfo, error) {
	cfg := boot.Current().GetLLMMgr().Config()
	m := cfg.ActiveModel()
	if m == nil {
...
func (s *ModelService) ListModels() ([]ModelInfo, error) {
	cfg := boot.Current().GetLLMMgr().Config()
...
func (s *ModelService) SetActiveModel(id string) error {
	...
	if err := boot.Current().GetLLMMgr().UpdateConfig(&newLLM); err != nil {
		return err
	}
...
func (s *ModelService) persistConfigFile() error {
	cfg := config.Get()
	if cfg == nil {
